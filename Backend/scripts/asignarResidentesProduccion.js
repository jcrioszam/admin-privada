...
async function asignarResidentesProduccion() {
  try {
    ...
    console.log('👥 Creando residentes para producción...');
    ...
    for (const residenteData of residentesData) {
      ...
      if (vivienda) {
        // Verificar si ya existe un residente para esta vivienda
        const residenteExistente = await Residente.findOne({ vivienda: vivienda._id });
        ...
        if (!residenteExistente) {
          ...
          console.log(`⚠️ Residente ya existe para Casa ${residenteData.numero}: ${residenteExistente.nombre} ${residenteExistente.apellidos}`);
        }
      } else {
