...
const Pagos = () => {
  ...
  const pagosAgrupados = useMemo(() => {
    ...
    // Agrupar por vivienda para evitar duplicados
    const agrupados = {};
    ...
    pagosFiltrados.forEach(pago => {
      ...
      // Calcular días de atraso para este pago
      const fechaLimite = new Date(pago.fechaLimite);
      const hoy = new Date();
      const diasAtraso = pago.estado === 'Pagado' || pago.estado === 'Pagado con excedente' || 
                         hoy <= fechaLimite ? 0 : Math.ceil((hoy - fechaLimite) / (1000 * 60 * 60 * 24));
      ...
      // Determinar estado general (si hay algún pago vencido, el estado es "Vencida")
      if (diasAtraso > 0) {
